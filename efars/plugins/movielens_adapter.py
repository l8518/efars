...
class MovieLensDatasetAdapter(DatasetAdapter):
    ...
    def generate(self, source_file_path, target_folder, seed, test_split_ratio, rating_threshold, max_user_n):
        ...
        testir_filepath = os.path.join(target_folder, "test_irrelevant_items.csv")
        ...
        users_reach_split = df_evt_cnt.loc[df_evt_cnt['cumsum'] <= trg_size_by_ratings]
        ...
        print("Amount of Ratings in whole Train + Test Datasets: {0}".format(len_tdf + len_trainset))
        print("Amount of Ratings in whole Dataset: {0}".format(df_len))
        # split test_df into relevant and non relevant items

        relevant_items = test_df.loc[test_df['rating'].astype('float64') >= rating_threshold]
        irrelevant_items = test_df.loc[test_df['rating'].astype('float64') < rating_threshold]
...
if __name__ == '__main__':
    ...
    mlds.generate("./data/ml-20m/ratings.csv", "./data/ratings/", 290219, 0.2, 3.5, 1700)
